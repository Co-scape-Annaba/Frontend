...
const getClassNames = (page) => {
  return page === pageActive ? "bg-primary bg-opacity-10 text-primary" : "";
};
    ...
    <div className="h-full flex flex-col items-center justify-between w-1/5 border-r-2 py-6">
      <div className=" flex flex-col gap-20 w-full px-10">
        <h2
          ...
          className="text-4xl font-bold mb-5 text-center font-racingSansOne text-primary flex gap-5 items-center justify-center cursor-pointer"
        >
        ...
        <ul className="flex flex-col gap-8 text-xl font-semibold mb-5 text-start">
          <a href="/dashboard">
            <li
              className={`rounded-xl w-full py-3 px-5 hover:bg-primary hover:bg-opacity-10 transition-all duration-200 flex gap-4
          ${getClassNames("/dashboard")} ${getClassNames("/simulate")}`}
              id="dashboard"
            >
              <img
                src={`icons/dashboard${getCurrentWord("/dashboard")}${getCurrentWord("/simulate")}.svg`}
                alt="dashboard icon"
              />
              <p>Dashboard</p>
            </li>
          ...
          <a href="/ChatBot">
            <li
              id="ChatBot"
              className={`rounded-xl w-full py-3 px-5 hover:bg-primary hover:bg-opacity-10 transition-all duration-200 flex gap-4
          ${getClassNames("/ChatBot")} ${getClassNames("/password")}`}
            >
              <img
                src={`icons/ChatBot${getCurrentWord("/ChatBot")}.svg`}
                alt="ChatBot icon"
              />
              <p>ChatBot</p>
            </li>{" "}
